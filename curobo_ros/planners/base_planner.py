#!/usr/bin/env python3
"""
Baseline reactive controller -- the thinnest possible ROS wrapper around
...
-------------------------------
``base_command_points`` (ROS param, default 4) points are published per solve,
...
class BaseController(ReactiveController):
    ...
    def build_solver(self):
        ...
        if not node.has_parameter('base_command_points'):
            node.declare_parameter('base_command_points', 4)
        self._command_points = max(2, int(node.get_parameter('base_command_points').value))
        ...
        node.get_logger().info(
            f"Base solver built: interpolation_dt={interpolation_dt}s, "
    ...
    def setup(self, start_state: JointState, goal_request: Any) -> bool:
        ...
        self.node.get_logger().info(
            f"Base: new goal - position=({raw[0]:.4f}, {raw[1]:.4f}, {raw[2]:.4f})m "
    ...
    def step(self, current_state: JointState) -> JointState:
        ...
        # points, so it cannot honour base_command_points > 4). The skipped
    ...
    def _csv_init(self):
        ...
        if not bool(self.node.get_parameter('mpc_debug').value):
            return
        self._csv = open_diag_csv(self.node, "base_diag")
