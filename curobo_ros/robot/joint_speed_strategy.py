...
class JointSpeedStrategy(JointCommandStrategy):
    ...
    def __init__(self, node, dt, description=None):
        ...
        self.dt = 0.02  # dt is defined by curobo interpolation
        ...
        # jerky/oscillating motion. self.dt (0.02s) matches leeloo's
        # execute_trajectory.cpp wall-timer cadence — the real point-to-point
        # interval on hardware — not the planner's own optimization_dt.
    ...
    def _clamp_velocities(self, vel_list):
        ...
        """
        max_dv = self._max_accel_rad_s2 * self.dt
        clamped = []
    ...
    def _debug_csv_write(self, vel_clamped):
        ...
        # est censé borner à max_dv = max_accel_rad_s2*self.dt par POINT, donc
        # ici sur le 1er point directement contre le réel).
        accel_p0 = deg(max(abs(a - b) for a, b in zip(p0, real)) / self.dt)
