...
# ============================================================
# PyTorch — pre-built wheel for SM_87 (Jetson Orin, aarch64)
#
...
#   pre-built wheels with SM_87 as of 2026-06.
#
# Built from source once (~3h, TORCH_CUDA_ARCH_LIST=8.7, USE_CUDNN=1,
# USE_MKLDNN=0) and cached on the build host at /data/wheels/ — see
# /data/wheels/README.md. Bind-mounted here (not COPYed) so the 263 MB wheel
# never enters the build context or an image layer.
#
# REQUIRES `docker build --build-context wheels=/data/wheels ...` (the
# `from=wheels` below names that additional context — a plain
# `source=/data/wheels` would resolve inside the build context, i.e. this
# docker/ directory, and fail with "not found").
# torchvision: NOT built — unused by curobo_ros and capacitynet.
#
# torchgen ships SEPARATELY as a tarball because the wheel does not contain it:
# PyTorch's setup.py installs torchgen as its own top-level directory next to
# torch/, and the from-source build that produced this wheel left it out. In
# torch 2.12 that is fatal, not cosmetic — torch/utils/_python_dispatch.py does
# a module-level `import torchgen`, reached from plain `import torch` via
# functional -> nn -> modules/module.py. Without it every `import torch` dies
# with ModuleNotFoundError. Pure Python + packaged YAML, so untarring into
# pip's site dir is enough; extracted from capacitynet:aarch64-jazzy, which
# carries the same torch commit (git7269437) built before the wheel was cut.
# ============================================================
ARG TORCH_WHEEL_FILENAME=torch-2.12.1a0+git7269437-cp312-cp312-linux_aarch64.whl
ARG TORCHGEN_TARBALL_FILENAME=torchgen-2.12.1a0+git7269437.tar.gz
RUN --mount=type=bind,from=wheels,source=.,target=/wheels,ro \
    pip3 install /wheels/${TORCH_WHEEL_FILENAME} && \
    tar xzf /wheels/${TORCHGEN_TARBALL_FILENAME} \
        -C "$(pip3 show torch | awk '/^Location:/{print $2}')"

# `import torch` MUST succeed — a failure here means a broken install, not a
# missing GPU. Keep it separate from the CUDA probe below: folding the two
# together lets `|| echo` swallow a genuine ImportError, which is exactly how
# a torch missing torchgen once shipped inside a "successful" build.
RUN python3 -c "import torch, torchgen; print('PyTorch', torch.__version__, '| torchgen OK')"

# GPU probe only — non-fatal, there is no GPU in the build sandbox.
RUN python3 -c "import torch; print('CUDA available:', torch.cuda.is_available())" || \
    echo "Warning: CUDA probe skipped (no GPU in build sandbox — expected)"
...
#
# `pip3 install --ignore-installed packaging` first: apt (software-properties-common's
# python3-packaging dep) installs packaging via dpkg, which leaves no RECORD file.
# When curobo's [dev] extras later pull in a newer packaging, pip tries to uninstall
# the dpkg-owned one and aborts with "Cannot uninstall packaging 24.0, RECORD file
# not found". Pre-upgrading it through pip alone gives it proper pip metadata so the
# main install below doesn't hit the same conflict.
RUN pip3 install --ignore-installed packaging && \
    MAX_JOBS=${MAKEFLAGS##*-j} pip3 install .[cu13,dev] --no-build-isolation
